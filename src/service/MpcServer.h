...
{
private:
    map<string, vector<string>> readerJournals; //一个reader有多个journal
    map<string, string> writerJournal;          //一个writer只有一个journal  一个journal  只有一个writer
...
private:
    ...
    bool registCom(const char *clientName, int &idx)
    {
        ...
        {
            if (find_if(sysJournalComIdxMap.cbegin(), sysJournalComIdxMap.cend(), [&i](const std::pair<string, int> &a)
                        { return a.second == i; }) == sysJournalComIdxMap.end())
            {
